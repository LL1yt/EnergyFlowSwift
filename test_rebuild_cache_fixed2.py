#!/usr/bin/env python3
"""Test cache rebuild with fixed parameters and proper initialization"""
...
def main():
    ...
    # Force cache initialization by calling precompute
    if hasattr(moe_processor, 'connection_classifier'):
        ...
        if hasattr(classifier, 'cache_manager') and classifier.cache_manager:
            logger.info("\n🔨 Forcing cache precomputation...")
            classifier.cache_manager.precompute_all_connections()
            
            cache_stats = classifier.cache_manager.get_cache_stats()
            logger.info(f"\n📦 Cache statistics:")
            logger.info(f"   Status: {cache_stats['status']}")
            if cache_stats['status'] != 'empty':
                logger.info(f"   Cached cells: {cache_stats.get('cached_cells', 0)}")
                logger.info(f"   Total connections: {cache_stats.get('total_connections', 0)}")
                logger.info(f"   Local connections: {cache_stats.get('local_connections', 0)}")
                logger.info(f"   Functional candidates: {cache_stats.get('functional_candidates', 0)}")
                logger.info(f"   Distant connections: {cache_stats.get('distant_connections', 0)}")
                logger.info(f"   Cache size: {cache_stats.get('cache_size_mb', 0):.1f} MB")
            else:
                logger.warning("   ⚠️ Cache is still empty after precomputation!")
