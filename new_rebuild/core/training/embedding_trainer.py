...
from ..common.interfaces import (
    EmbeddingProcessor, CubeInterface, TrainingInterface,
...
class EmbeddingTrainer(TrainingInterface):
    ...
    def __init__(self, config: SimpleProjectConfig):
        ...
        self.training_history = {
            'losses': [], 'reconstruction_losses': [], 'similarity_losses': [],
            'diversity_losses': [], 'emergence_losses': [], 'val_scores': []
        }
    ...
    def _init_components(self):
        ...
        logger.info(f"EmbeddingTransformer: {self.embedding_transformer.get_parameter_count()} параметров")
        
        # 2. MoE Connection Processor (основной куб)
        lattice_dims = (
            self.config.embedding_training.test_lattice_dim,
            self.config.embedding_training.test_lattice_dim, 
            self.config.embedding_training.test_lattice_dim
        )
        ...
        if self.config.embedding_training.test_mode:
    ...
    def train_epoch(self, dataloader: DataLoader, optimizer=None, **kwargs) -> Dict[str, float]:
        ...
        epoch_losses = {
            'total': 0.0, 'reconstruction': 0.0, 'similarity': 0.0,
            'diversity': 0.0, 'emergence': 0.0, 'count': 0
        }
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if isinstance(batch, dict):
                ...
                target_embeddings = batch.get('target_embeddings', input_embeddings).to(self.device)
                texts = batch.get('texts', None)
            elif isinstance(batch, (list, tuple)):
                input_embeddings = batch[0].to(self.device)
                target_embeddings = batch[1].to(self.device) if len(batch) > 1 else input_embeddings
    ...
    def _forward_pass(self, input_embeddings: torch.Tensor, 
        ...
        # Для простоты пока используем cube_embeddings как состояния клеток
        # В полной реализации нужно будет интегрировать с lattice структурой
        processed_embeddings = self.moe_processor(cube_embeddings)
        
        # 3. Cube → Teacher (64D → 768D обратно)
        output_embeddings = self.embedding_transformer.transform_from_cube(processed_embeddings)
    ...
    def _compute_losses(self, input_embeddings: torch.Tensor, 
        ...
        reconstruction_loss = nn.functional.mse_loss(output_embeddings, target_embeddings)
        losses['reconstruction'] = reconstruction_loss * self.config.embedding_training.reconstruction_loss_weight
        
        # 2. Similarity Loss (cosine similarity preservation)
        input_sim = torch.cosine_similarity(input_embeddings.unsqueeze(1), input_embeddings.unsqueeze(0), dim=2)
        output_sim = torch.cosine_similarity(output_embeddings.unsqueeze(1), output_embeddings.unsqueeze(0), dim=2)
        similarity_loss = nn.functional.mse_loss(output_sim, input_sim)
        losses['similarity'] = similarity_loss * self.config.embedding_training.similarity_loss_weight
        ...
        losses['diversity'] = diversity_loss * self.config.embedding_training.diversity_loss_weight
        ...
        emergence_loss = -torch.log(identity_loss + 1e-8)  # Логарифмическое поощрение различий
        losses['emergence'] = emergence_loss * self.config.embedding_training.emergence_loss_weight
    ...
    def load_checkpoint(self, path: str) -> Dict[str, Any]:
        ...
        if 'text_decoder' in checkpoint and hasattr(self.text_decoder, 'load_state_dict'):
