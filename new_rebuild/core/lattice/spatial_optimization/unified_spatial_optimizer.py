...
class UnifiedSpatialOptimizer:
    ...
    def __init__(
        ...
        self.chunker = (
            self.gpu_processor.chunker if self.config.enable_adaptive_chunking else None
        )
        if self.chunker:
            logger.info(
                f"[UnifiedSpatialOptimizer] Используется chunker из GPUSpatialProcessor с {len(self.chunker._chunks)} chunks"
            )
    ...
    def _create_moe_processor_fn(
        ...
        """
        Создает processor function для MoE архитектуры (ОПТИМИЗИРОВАННАЯ ВЕРСИЯ)
        
        В новой архитектуре neighbor_states и neighbor_indices больше не передаются,
        так как они получаются из кэша внутри MoE processor.
        """
        ...
        def moe_processor(current_state, neighbor_states, cell_idx, neighbor_indices):
            try:
                ...
                logger.error(
                    f"🔍 Context: cell_idx={cell_idx}, current_state.shape={getattr(current_state, 'shape', 'N/A')}"
                )
                return (
