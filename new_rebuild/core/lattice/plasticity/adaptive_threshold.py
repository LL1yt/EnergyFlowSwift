"""
Адаптивные пороги активности для BCM правила - Clean Implementation
================================================================
...
Адаптировано для интеграции с HybridCellV2.
"""
...
class AdaptiveThreshold:
    ...
    def __init__(
        ...
        logger.info(f"AdaptiveThreshold initialized for {total_cells} cells")
        logger.info(
    ...
    def get_plasticity_factor(
        ...
        # Проверяем размеры и адаптируем если необходимо
        if post_activity.shape[0] != self.thresholds.shape[0]:
            # Если размеры не совпадают, используем соответствующее количество порогов
            num_cells = min(post_activity.shape[0], self.thresholds.shape[0])
            post_activity_subset = post_activity[:num_cells]
            thresholds_subset = self.thresholds[:num_cells]
        else:
            post_activity_subset = post_activity
            thresholds_subset = self.thresholds

        # BCM правило: (post - threshold)
        # Если post > threshold → LTP (positive plasticity)
        # Если post < threshold → LTD (negative plasticity)
        plasticity_factor = post_activity_subset - thresholds_subset
    ...
    def reset_thresholds(self, initial_threshold: Optional[float] = None):
        ...
        logger.info(f"Thresholds reset to {initial_threshold}")
