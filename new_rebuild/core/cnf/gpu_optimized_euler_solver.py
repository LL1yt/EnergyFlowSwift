...
class GPUOptimizedEulerSolver(nn.Module):
    ...
    def __init__(self, config: Optional[SolverConfig] = None):
        ...
        self.lipschitz_factor = nn.Parameter(torch.tensor(self.config.lipschitz_safety_factor))
    ...
    def _get_memory_pool_tensor(self, shape: Tuple[int, ...], dtype: torch.dtype) -> torch.Tensor:
    ...
    def _estimate_lipschitz_constant(
        ...
        *args, **kwargs
        ...
        perturbations = torch.randn_like(states) * epsilon
    ...
    def batch_euler_step(
        ...
        *args, **kwargs
        ...
        if invalid_mask.any():
            logger.warning(f"Invalid derivatives detected: {invalid_mask.sum().item()}/{batch_size}")
            # Заменяем invalid derivatives на нули
            derivatives = torch.where(
                invalid_mask.unsqueeze(-1), 
                torch.zeros_like(derivatives), 
                derivatives
            )
        ...
        result_invalid_mask = torch.isnan(next_states).any(dim=-1) | torch.isinf(next_states).any(dim=-1)
        
        if result_invalid_mask.any():
            logger.warning(f"Invalid next_states detected: {result_invalid_mask.sum().item()}/{batch_size}")
            # Возвращаем original states для invalid cases
            next_states = torch.where(
                result_invalid_mask.unsqueeze(-1),
                states,
                next_states
            )
    ...
    def batch_integrate(
        ...
        *args, **kwargs
        ...
        if return_trajectory:
            ...
            trajectory = self._get_memory_pool_tensor(trajectory_shape, initial_states.dtype)
        ...
        for step in range(num_steps):
            t = torch.tensor(step * base_dt, device=self.device, dtype=initial_states.dtype)
            ...
            if self.config.adaptive_method == AdaptiveMethod.LIPSCHITZ_BASED:
                ...
                derivatives = derivative_fn(t.expand(batch_size), current_states, *args, **kwargs)
                ...
                derivative_magnitude = torch.norm(derivatives, dim=-1)
                violations = (derivative_magnitude > self.config.stability_threshold).sum().item()
        ...
        self._update_performance_stats(
            integration_time_ms, num_steps, total_adaptive_adjustments, 
            total_stability_violations, batch_size, memory_usage_mb
        )
        ...
        *args, **kwargs
        ...
        while steps_taken < max_steps and active_mask.any():
            ...
            half_step1, _ = self.batch_euler_step(
                derivative_fn, batch_current_states, batch_time, half_dt, *args, **kwargs
            )
            half_step2, _ = self.batch_euler_step(
                derivative_fn, half_step1, batch_time + half_dt, half_dt, *args, **kwargs
            )
    ...
    def _update_performance_stats(
        ...
        new_avg = (old_avg * (total_integrations - 1) + integration_time_ms) / total_integrations
        ...
        self.performance_stats["batch_efficiency"] = batch_size / max(1, integration_time_ms / 1000)
        
        # Memory usage
        self.performance_stats["gpu_memory_peak_mb"] = max(
            self.performance_stats["gpu_memory_peak_mb"], 
            memory_usage_mb
        )
...
def batch_euler_solve(
    ...
    *args, **kwargs
    ...
    result = solver.batch_integrate(
        derivative_fn, initial_states, integration_time, num_steps, 
        return_trajectory, *args, **kwargs
    )
...
def benchmark_solver_performance(
    ...
    for batch_size in batch_sizes:
        ...
        results[f"batch_{batch_size}"] = {
            ...
            "memory_efficiency_mb_per_trajectory": avg_memory / batch_size if batch_size > 0 else 0
        }
