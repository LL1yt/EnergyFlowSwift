...
class GPUEnhancedCNF(nn.Module):
    ...
    def _create_derivative_function(self, neighbor_influences: torch.Tensor):
        ...
        def derivative_fn(t: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
    ...
    def _process_single_connection(
        ...
        """Legacy single connection processing"""
        # Агрегация соседей (простое среднее)
        if neighbor_states.shape[0] > 0:
            neighbor_influence = neighbor_states.mean(dim=0, keepdim=True)
        else:
            neighbor_influence = torch.zeros_like(current_state)
        
        # Создаем derivative function
        derivative_fn = self._create_derivative_function(neighbor_influence)
    ...
    def _process_connection_batch(
        self,
        current_states: torch.Tensor,
        neighbor_states_list: List[torch.Tensor]
    ) -> torch.Tensor:
        """Batch processing для множественных connections"""
    ...
    def _determine_optimal_batch_size(self, total_connections: int) -> int:
        ...
        if self.batch_processing_mode == BatchProcessingMode.SINGLE:
            ...
            memory_per_connection = self.state_size * 4 * 10 / (1024**2)  # очень грубая оценка
            
            max_affordable = int(available_mb * 0.5 / memory_per_connection)  # 50% от доступной памяти
            optimal_batch_size = min(total_connections, self.max_batch_size, max_affordable)
    ...
    def forward(
        ...
        **kwargs
        ...
        if isinstance(neighbor_states, list):
            # Batch mode - список neighbor_states для разных connections
            ...
            if optimal_batch_size >= batch_size:
                ...
                integration_result = self._process_connection_batch(current_state, neighbor_states)
                ...
                for i in range(0, batch_size, optimal_batch_size):
                    ...
                    batch_neighbors = neighbor_states[i:end_idx]
                    
                    batch_result = self._process_connection_batch(batch_current, batch_neighbors)
            ...
            result_state = self._process_single_connection(current_state, neighbor_states)
    ...
    def _update_performance_stats(self, processing_time_ms: float, batch_size: int):
        ...
        new_avg_time = (old_avg_time * (total_passes - 1) + processing_time_ms) / total_passes
        ...
        self.performance_stats["batch_efficiency"] = batch_size / (processing_time_ms / 1000)
        
        # GPU memory usage
        device_stats = self.device_manager.get_memory_stats()
        self.performance_stats["gpu_memory_usage_mb"] = device_stats.get("allocated_mb", 0)
...
def benchmark_cnf_performance(
    ...
    for state_size in state_sizes:
        for batch_size in batch_sizes:
            logger.info(f"🧪 Бенчмарк CNF: state_size={state_size}, batch_size={batch_size}")
            ...
            avg_processing_time = sum(r["processing_time_ms"] for r in trial_results) / num_trials
