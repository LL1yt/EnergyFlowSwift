...
def test_imports():
    ...
    try:
        from core.cnf.gpu_optimized_euler_solver import (
...
def test_adaptive_methods():
    ...
    try:
        ...
        def complex_derivative(t: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
            ...
            dxdt = -0.1 * x + 0.5 * y * torch.sin(t.unsqueeze(-1) if t.dim() == 1 else t)
            dydt = -0.2 * y - 0.3 * x * torch.cos(t.unsqueeze(-1) if t.dim() == 1 else t)
            
            # Добавляем остальные размерности как damped
            rest = -0.1 * states[..., 2:] if states.shape[-1] > 2 else torch.empty(states.shape[0], 0, device=states.device)
        ...
        for method in methods_to_test:
            ...
            logger.info(f"     📊 Final norm: {results[method.value]['final_norm']:.3f}")
        ...
        for method, result in results.items():
            logger.info(f"     {method}: {result['integration_time_ms']:.1f}ms, "
...
def test_batch_processing():
    ...
    try:
        from core.cnf.gpu_optimized_euler_solver import create_gpu_optimized_euler_solver
        ...
        for batch_size in batch_sizes:
            ...
            # Создаем solver
            solver = create_gpu_optimized_euler_solver(max_batch_size=max(100, batch_size))
        ...
        baseline_throughput = performance_results[baseline_batch]["trajectories_per_second"]
        
        for batch_size in batch_sizes:
            if batch_size == baseline_batch:
                continue
                
            current_throughput = performance_results[batch_size]["trajectories_per_second"]
            speedup = current_throughput / baseline_throughput
            efficiency = speedup / (batch_size / baseline_batch)
            
            logger.info(f"     Batch {batch_size}: {speedup:.1f}x speedup, "
...
def test_adaptive_integration():
    ...
    try:
        ...
        if result.trajectory is not None:
            ...
            logger.info(f"     ⚡ Fast modes: {initial_fast_norm:.4f} → {final_fast_norm:.4f}")
            logger.info(f"     🐌 Slow modes: {initial_slow_norm:.4f} → {final_slow_norm:.4f}")
            ...
            logger.info(f"     📉 Decay ratios - Fast: {fast_decay_ratio:.4f}, "
        ...
        if result.error_estimates:
            ...
            logger.info(f"     📊 Error estimates - Avg: {avg_error:.2e}, Max: {max_error:.2e}")
...
def test_performance_benchmark():
    ...
    try:
        from core.cnf.gpu_optimized_euler_solver import benchmark_solver_performance
        ...
        for batch_key, metrics in results.items():
            ...
            logger.info(f"       🚀 Throughput: {metrics['throughput_trajectories_per_second']:.0f} traj/s")
            logger.info(f"       📊 Efficiency: {metrics['memory_efficiency_mb_per_trajectory']:.2f} MB/traj")
            logger.info(f"       ✅ Success: {metrics['success_rate']:.1%}")
        
        # Анализ efficiency scaling
        batch_sizes = [int(k.split('_')[1]) for k in results.keys()]
        throughputs = [results[f"batch_{bs}"]["throughput_trajectories_per_second"] for bs in batch_sizes]
        
        if len(batch_sizes) > 1:
            logger.info("   📈 Throughput scaling:")
            baseline_throughput = throughputs[0] / batch_sizes[0]  # per-trajectory throughput
            ...
            for i, (bs, throughput) in enumerate(zip(batch_sizes, throughputs)):
                ...
                logger.info(f"     Batch {bs}: {scaling_efficiency:.2f}x efficiency vs single trajectory")
...
def test_memory_efficiency():
    ...
    try:
        ...
        for config_name, memory_efficient in configs:
            ...
            solver = create_gpu_optimized_euler_solver(memory_efficient=memory_efficient)
            ...
            results[config_name] = {
                "avg_memory_per_integration": np.mean([r.memory_usage_mb for r in integration_results]),
                "memory_pool_size": stats["memory_pool"]["pool_size"],
                "peak_memory_mb": stats["performance"]["gpu_memory_peak_mb"]
            }
            
            logger.info(f"     📊 Memory pool size: {stats['memory_pool']['pool_size']}")
            logger.info(f"     💾 Peak memory: {stats['performance']['gpu_memory_peak_mb']:.1f}MB")
        ...
        for config_name, metrics in results.items():
            ...
            logger.info(f"       📊 Avg memory/integration: {metrics['avg_memory_per_integration']:.1f}MB")
