...
│   │   ├── hybrid_connection_processor.py ✅ MoE с 3 экспертами
...
- Адаптивные соседи: Адаптивный радиус в зависимости от размера решетки
  max*dim = max(размеры 3д сетки)
  adaptive_radius = max_dim * 0.3 30% от размера

            all_neighbors = optimizer.find_neighbors_optimized(
                cell_coords, radius=adaptive_radius
            ) - проверить реализацию, так как у нас уже есть в коде ограничение по соседям, но оно сделано просто количеством соседей, а тут именно радиус определяется охвата соседей вокруг клетки, если я правильно понял - так вто нужно проверить, правильно ли реализовано и не ломает ли нашу трех уровневую систему?
