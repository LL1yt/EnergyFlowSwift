...
def test_vectorized_neural_ode():
    ...
    try:
        ...
        logger.info(
            f"   📊 Параметры: {sum(p.numel() for p in neural_ode.parameters())}"
        )
        ...
        logger.info(
            f"   📊 Входные данные: states={current_states.shape}, neighbors={neighbor_influences.shape}"
        )
        ...
        logger.info(
            f"     🔍 Статистика: mean={derivatives.mean().item():.4f}, std={derivatives.std().item():.4f}"
        )
...
def test_gpu_enhanced_cnf_basic():
    ...
    try:
        ...
        logger.info(
            f"   📈 CNF stats: {stats['cnf_performance']['total_forward_passes']} passes"
        )
...
def test_cnf_performance_scaling():
    ...
    try:
        from new_rebuild.core.cnf import create_gpu_enhanced_cnf, BatchProcessingMode
...
def test_adaptive_methods_comparison():
    ...
    try:
        ...
        methods = [AdaptiveMethod.LIPSCHITZ_BASED, AdaptiveMethod.ACTIVITY_BASED]
        ...
        for method in methods:
            ...
            results[method.value] = {
                ...
                "stability_violations": solver_stats["performance"][
                    "stability_violations"
                ],
                "adaptive_adjustments": solver_stats["performance"][
            ...
            logger.info(
                f"     🔧 Adaptive adjustments: {results[method.value]['adaptive_adjustments']}"
            )
            logger.info(
                f"     ⚠️ Stability violations: {results[method.value]['stability_violations']}"
            )
            logger.info(
                f"     📊 Final norm: {results[method.value]['final_state_norm']:.3f}"
            )
...
def test_cnf_benchmark():
    ...
    try:
        ...
        for key, metrics in results.items():
            ...
            logger.info(
                f"       📈 Throughput: {metrics['throughput_connections_per_second']:.0f} conn/s"
            )
        ...
        if state_16_results and state_32_results:
            ...
            for batch_size in [1, 10, 20]:
                ...
                if key_16 in results and key_32 in results:
                    ...
                    logger.info(
                        f"     Batch {batch_size}: 16D/32D = {ratio:.2f}x throughput"
                    )
...
def test_integration_with_moe():
    ...
    try:
        ...
        logger.info(
            f"   📊 Распределение: {distant_connections} distant, {functional_connections} functional"
        )
        ...
        total_processing_time = (
            distant_result["processing_time_ms"]
            + functional_result["processing_time_ms"]
        )
        ...
        logger.info(
            f"     Distant: {distant_stats['cnf_performance']['batch_efficiency']:.1f} conn/s"
        )
        logger.info(
            f"     Functional: {functional_stats['cnf_performance']['batch_efficiency']:.1f} conn/s"
        )
