...
def create_synthetic_embedding_dataset(config: SimpleProjectConfig, num_samples: int = 100):
    ...
    for i in range(num_samples):
        ...
        pattern = torch.sin(torch.arange(embedding_dim, dtype=torch.float32) * 
                           (i / num_samples) * np.pi * 2) * 0.05
    ...
    logger.info(f"Статистика эмбедингов: mean={embeddings.mean():.4f}, std={embeddings.std():.4f}")
...
def test_trainer_basic():
    ...
    config.embedding_training.test_mode = True
    config.embedding_training.test_lattice_dim = 8  # Малый куб 8×8×8
    config.embedding_training.test_quick_iterations = 5
    
    # Инициализация тренера
    trainer = create_embedding_trainer(config)
    
    print(f"✓ Тренер инициализирован на устройстве: {trainer.device}")
    print(f"✓ Параметров в EmbeddingTransformer: {trainer.embedding_transformer.get_parameter_count()}")
...
def test_performance_monitoring(trainer):
    ...
    if summary['performance_stats']['avg_total_time'] > 0:
        print(f"✓ Среднее время батча: {summary['performance_stats']['avg_total_time']:.4f}s")
...
def main():
    ...
    try:
        ...
        # Итоговая статистика
        print(f"\nИтоговая статистика:")
        print(f"  Архитектура: DistilBERT → EmbeddingTransformer → MoE Cube → TextDecoder")
        print(f"  Решетка: {trainer.config.embedding_training.test_lattice_dim}×{trainer.config.embedding_training.test_lattice_dim}×{trainer.config.embedding_training.test_lattice_dim}")
