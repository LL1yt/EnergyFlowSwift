...
class ConfigManager:
    ...
    def _check_file_changes(self):
        ...
        if changed_files:
            self.logger.info(
                f"🔄 Detected changes in {len(changed_files)} config files"
            )
            self.reload_config()
    ...
    def _initialize_dynamic_config(self):
        ...
        try:
            self.logger.info("🔧 Initializing Dynamic Configuration System...")
        ...
        try:
            ...
            for section_name, section_data in dynamic_config.items():
                ...
                if section_name in self._config_cache:
                    ...
                    self.logger.debug(
                        f"🔄 Merged dynamic config for section: {section_name}"
                    )
                else:
