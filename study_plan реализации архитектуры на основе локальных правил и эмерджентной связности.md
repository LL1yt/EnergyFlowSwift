...
**Приоритет:** КРИТИЧНЫЙ - Блокирует масштабирование до 300×300×150

**Текущая проблема:** 54,892 параметра → необходимо 10,000 (82% reduction)

- [ ] **Анализ архитектуры GatedMLPCell:**

  - [ ] Профилирование размеров каждого слоя
  - [ ] Определение критически важных компонентов
  - [ ] Оценка влияния каждого параметра на производительность

- [ ] **Стратегия оптимизации:**

  - [ ] Уменьшение hidden_dim: 64 → 24-32
  - [ ] Упрощение gating mechanism: возможно linear gating
  - [ ] Временное отключение memory component
  - [ ] Shared weights для однотипных операций

- [ ] **Тестирование оптимизированной версии:**

  - [ ] Валидация количества параметров: ≤ 10,000
  - [ ] Тестирование производительности forward pass
  - [ ] Сравнение с baseline GatedMLPCell
  - [ ] Проверка совместимости с connection_weights

- [ ] **Создание конфигурации optimized_gated_mlp.yaml:**
  - [ ] Оптимизированные параметры архитектуры
  - [ ] Target 300×300×150 с 10,000 params для GatedMLP
  - [ ] Интеграция с hybrid_nca_gmlp.yaml

**Критерии успеха:**

- Параметры: ≤ 10,000 (vs текущих 54,892)
- Производительность: ≤ 250ms forward pass для 13.5M connections
- Совместимость: полная интеграция с MinimalNCACell
- Memory: умещение 300×300×150 решетки в 24GB VRAM

### Шаг 2.3: STDP механизм [ПЛАНИРУЕТСЯ ПОСЛЕ 2.2]

**Приоритет:** ВЫСОКИЙ - Ключевой механизм самоорганизации (ЗАБЛОКИРОВАНО до 2.2)
