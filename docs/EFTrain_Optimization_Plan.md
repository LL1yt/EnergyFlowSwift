...
- В соответствии с Analysis*: warmupSteps ~ 500–1000, clipNorm ~ 1–2, microBatch максимально возможный по памяти GPU.
- В TrainConfig уже есть поля; обновить sample-конфиг и README-заметки.

17) Токенизатор и данные
- Для качества: переход на BPE/SentencePiece, swift-tokenizers или TokenizerKit (см. TextBridge план). Стабильная вокабуляризация нужна для «обратимости» Text↔Cube↔Text.
