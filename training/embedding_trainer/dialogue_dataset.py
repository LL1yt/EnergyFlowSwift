...
class DialogueConfig:
    ...
    semantic_similarity_threshold: float = (
        0.3  # Минимальная семантическая связность Q&A
    )
...
def map_model_name_to_key(model_name: str) -> str:
    ...
    print(
        f"[WARNING] Model '{model_name}' not found in SUPPORTED_LLM_MODELS, using 'distilbert' as fallback"
    )
    return "distilbert"
...
class DialogueDataset(Dataset):
    ...
    def __init__(
        ...
        if not EMBEDDING_LOADER_AVAILABLE:
            raise ImportError(
                "EmbeddingLoader is required for DialogueDataset. "
        ...
        if question_embeddings is not None and answer_embeddings is not None:
            ...
            raise ValueError(
                "No dialogue data provided. Specify dialogue_pairs, conversations, "
        ...
        self.logger.info(
            f"   Quality filtered: {self.cache_stats['quality_filtered']} pairs"
        )

    def _load_config(
        self, config: Optional[Union[DialogueConfig, Dict, str]]
    ) -> DialogueConfig:
        ...
        if config is None:
            ...
            self.logger.warning(
                f"Unknown config type: {type(config)}. Using default config."
            )
            return DialogueConfig()
    ...
    def _validate_teacher_model(self):
        ...
        try:
            ...
            self.logger.warning(
                f"[WARNING]  Teacher model {self.config.teacher_model} not available: {e}"
            )

            # Пробуем fallback
            fallback_key = map_model_name_to_key(self.config.fallback_model)
            self.logger.info(
                f"Switching to fallback model: {self.config.fallback_model} (key: {fallback_key})"
            )
            self.config.teacher_model = fallback_key

    def _load_from_embeddings(
        ...
        if not isinstance(question_embeddings, torch.Tensor):
            question_embeddings = torch.from_numpy(
    ...
    def _load_from_dialogue_pairs(self, dialogue_pairs: List[Dict]):
        ...
        if cached_data is not None and self.config.use_cache:
            ...
            self.logger.info(
                f"Generating embeddings from {len(dialogue_pairs)} dialogue pairs "
                f"using Teacher LLM: {self.config.teacher_model}"
            )

            # Загружаем эмбединги батчами
            ...
            question_embeddings = self.embedding_loader.batch_load_from_llm(
                questions, model_key=model_key, batch_size=self.config.cache_batch_size
            )
            answer_embeddings = self.embedding_loader.batch_load_from_llm(
                answers, model_key=model_key, batch_size=self.config.cache_batch_size
            )

            # Обновляем эмбединги и метаданные
            self._load_from_embeddings(question_embeddings, answer_embeddings)
    ...
    def _load_from_conversations(self, conversations: List[List[Dict]]):
        ...
        self.logger.info(
            f"Extracted {len(dialogue_pairs)} Q&A pairs from conversations"
        )
    ...
    def _extract_qa_pairs_from_conversation(
        ...
        for i in range(len(conversation) - 1):
            ...
            if current_turn.get("role", "").lower() in [
                "user",
                "human",
                "question",
            ] and next_turn.get("role", "").lower() in ["assistant", "bot", "answer"]:

                pairs.append(
                    {
                        "question": current_turn.get(
                            "text", current_turn.get("content", "")
                        ),
    ...
    def _load_from_sources(self):
        ...
        for source_path in self.config.data_sources:
            ...
            try:
                ...
                self.logger.info(
                    f"Loaded {len(pairs)} dialogue pairs from {source_path}"
                )
    ...
    def _load_dialogues_from_json(self, file_path: Path) -> List[Dict]:
        ...
        for dialogue in dialogues:
            if "question" in dialogue and "answer" in dialogue:
                # Простой Q&A формат
                pairs.append(
                    {"question": dialogue["question"], "answer": dialogue["answer"]}
                )
            elif "conversations" in dialogue:
                # Многоходовой формат
                conversation_pairs = self._extract_qa_pairs_from_conversation(
    ...
    def _filter_dialogue_quality(self, dialogue_pairs: List[Dict]) -> List[Dict]:
        ...
        for pair in dialogue_pairs:
            ...
            # Проверка длины
            if (
                len(question) < self.config.min_question_length
                or len(answer) < self.config.min_answer_length
                or len(question) > self.config.max_question_length
                or len(answer) > self.config.max_answer_length
            ):
                filtered_count += 1
                continue
            ...
            if (
                not question.strip()
                or not answer.strip()
                or question.strip() == answer.strip()
            ):
                filtered_count += 1
                continue
        ...
        self.logger.info(
            f"Quality filter: kept {len(filtered_pairs)}, filtered {filtered_count}"
        )
    ...
    def _create_cache_key_for_dialogues(self, dialogue_pairs: List[Dict]) -> str:
        ...
        dialogue_content = "\n".join(
            [f"Q:{d['question']} A:{d['answer']}" for d in sample_dialogues]
        )
    ...
    def get_sample_dialogues(self, n_samples: int = 5) -> Dict[str, Any]:
        ...
        for idx in sample_indices:
            ...
            cosine_similarity = torch.cosine_similarity(
                question_emb.unsqueeze(0), answer_emb.unsqueeze(0)
            ).item()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        qa_similarities = torch.cosine_similarity(
            self.question_embeddings, self.answer_embeddings, dim=1
        )
        ...
        stats.update(
            {
                "embedding_quality": {
                    ...
                    "qa_similarity_range": [
                        qa_similarities.min().item(),
                        qa_similarities.max().item(),
                    ],
                }
            }
        )
    ...
    def __repr__(self):
        return (
            f"DialogueDataset(pairs={len(self.question_embeddings)}, "
...
def create_dialogue_dataset(
    ...
    **kwargs,
    ...
    """
    Создает DialogueDataset из списка диалоговых пар.

    Args:
        dialogue_pairs: Список диалоговых пар.
        teacher_model: Название LLM-модели для генерации эмбедингов.
        validation_split: Доля данных для валидации.
        cache_embeddings: Включить ли кэширование эмбедингов.
        **kwargs: Дополнительные параметры для DialogueConfig.

    Returns:
        DialogueDataset
    """
    # Создаем конфиг, передавая ключевые аргументы
...
def create_conversation_dataset(
    ...
    **kwargs,
    ...
    config = DialogueConfig(
        teacher_model=teacher_model, validation_split=validation_split, **kwargs
    )

    return DialogueDataset(config=config, conversations=conversations)


def load_dialogue_dataset_from_files(
    file_paths: List[str],
