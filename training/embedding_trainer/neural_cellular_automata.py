...
class StochasticCellUpdater(nn.Module):
    ...
    def __init__(self, config: NCAConfig, cube_dimensions: Tuple[int, int, int]):
        ...
        self.register_buffer('update_history', torch.zeros(self.total_cells, dtype=torch.float32))
        self.register_buffer('last_update_step', torch.zeros(self.total_cells, dtype=torch.long))
        
        logger.info(f"🎲 StochasticCellUpdater initialized: {self.total_cells} cells, p={self.update_probability}")
    
    def generate_update_mask(self, batch_size: int, current_step: int = 0) -> torch.Tensor:
        """
        ...
        if self.config.stochastic_scheduling:
            ...
            prob_matrix = torch.full((batch_size, depth, height, width), 
            ...
            if current_step > 0:
                ...
                prob_matrix = torch.clamp(prob_matrix, 0.1, 0.9)  # Keep reasonable bounds
            
            update_mask = torch.bernoulli(prob_matrix).bool()
        else:
            # Fallback: deterministic mask
            update_mask = torch.ones(batch_size, depth, height, width, 
                                   dtype=torch.bool, 
                                   device=self.update_history.device)
    ...
    def forward(self, 
                ...
                current_step: int = 0) -> torch.Tensor:
        """
...
class ResidualUpdateRules(nn.Module):
    ...
    def __init__(self, config: NCAConfig, state_size: int = 32):
        ...
        logger.info(f"🔄 ResidualUpdateRules initialized: state_size={state_size}, lr={self.residual_lr}")
    
    def forward(self, 
                current_states: torch.Tensor, 
                raw_updates: torch.Tensor) -> torch.Tensor:
        """
        ...
        if magnitude_mask.any():
            ...
            scaling_factor = torch.where(magnitude_mask, scaling_factor, torch.ones_like(scaling_factor))
    ...
    def get_update_magnitude_stats(self, 
                                 current_states: torch.Tensor, 
                                 updates: torch.Tensor) -> Dict[str, float]:
...
class PatternFormationMetrics(nn.Module):
    ...
    def __init__(self, config: NCAConfig, cube_dimensions: Tuple[int, int, int]):
        ...
        self.register_buffer('pattern_history', torch.zeros(100, dtype=torch.float32))  # Last 100 steps
        self.register_buffer('specialization_history', torch.zeros(100, dtype=torch.float32))
        self.step_counter = 0
    ...
    def compute_spatial_coherence(self, cube_states: torch.Tensor) -> torch.Tensor:
        ...
        for dz, dy, dx in directions:
            ...
            if (0 <= dz + self.depth - 1 < self.depth and 
                0 <= dy + self.height - 1 < self.height and 
                0 <= dx + self.width - 1 < self.width):
        ...
        if coherence_scores:
            ...
            logger.warning("⚠️ [NCA] No valid neighbors found for spatial coherence computation")
            spatial_coherence = torch.tensor(0.5, device=cube_states.device)  # Нейтральное значение
        
        return spatial_coherence
    
    def compute_emergent_specialization(self, cube_states: torch.Tensor) -> torch.Tensor:
        """
        ...
        mid_depth, mid_height, mid_width = self.depth // 2, self.height // 2, self.width // 2
        ...
        for region_name, region_states in regions.items():
            region_signatures[region_name] = region_states.mean(dim=(1, 2, 3))  # [batch, state_size]
        ...
        if specialization_scores:
            ...
            logger.warning("⚠️ [NCA] No valid regions found for specialization computation")
            specialization = torch.tensor(0.1, device=cube_states.device)  # Низкая специализация по умолчанию
    ...
    def forward(self, cube_states: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.step_counter > 5:  # Need some history
            recent_specialization = self.specialization_history[max(0, self.step_counter-5):self.step_counter]
            temporal_consistency = 1.0 - torch.std(recent_specialization) / (torch.mean(recent_specialization) + 1e-8)
    ...
    def get_pattern_summary(self) -> Dict[str, Any]:
        ...
        recent_specialization = self.specialization_history[max(0, self.step_counter-recent_steps):self.step_counter]
        
        return {
            "current_specialization": self.specialization_history[self.step_counter-1].item() if self.step_counter > 0 else 0.0,
            "avg_specialization": recent_specialization.mean().item(),
            "specialization_trend": (recent_specialization[-5:].mean() - recent_specialization[:5].mean()).item() if recent_steps >= 10 else 0.0,
            "specialization_stability": 1.0 / (1.0 + recent_specialization.std().item()),
...
class NeuralCellularAutomata(nn.Module):
    ...
    def __init__(self, 
        ...
        logger.info(f"🧠 NeuralCellularAutomata initialized: {cube_dimensions} cube, {state_size}D states")
    
    def forward(self, 
                current_states: torch.Tensor,
                raw_updates: torch.Tensor,
                enable_stochastic: bool = True,
                enable_residual: bool = True) -> Dict[str, torch.Tensor]:
        """
        ...
        if current_states.shape != raw_updates.shape:
            logger.error(f"❌ [NCA] Shape mismatch: current_states {current_states.shape} vs raw_updates {raw_updates.shape}")
            raise ValueError(f"Shape mismatch in NCA inputs: {current_states.shape} vs {raw_updates.shape}")
        ...
        if enable_residual:
            ...
            results['residual_stats'] = self.residual_rules.get_update_magnitude_stats(current_states, raw_updates)
        else:
            refined_updates = raw_updates
        
        # 2. Apply stochastic cell updates (если enabled)
        if enable_stochastic:
            final_states = self.stochastic_updater(current_states, refined_updates, self.training_step)
        ...
        results['state_change_magnitude'] = torch.norm(final_states - current_states, dim=-1).mean()
    ...
    def get_nca_summary(self) -> Dict[str, Any]:
        ...
        if hasattr(self.stochastic_updater, 'get_update_statistics'):
            summary["stochastic_stats"] = self.stochastic_updater.get_update_statistics()
...
def test_nca_basic() -> bool:
    ...
    try:
        ...
        current_states = torch.randn(batch_size, 6, 8, 8, 16)  # [batch, depth, height, width, state_size]
