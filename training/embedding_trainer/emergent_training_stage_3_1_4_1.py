...
from .neural_cellular_automata import (
...
class EmergentMultiObjectiveLoss(nn.Module):
    ...
    def __init__(self, config: EmergentTrainingConfig):
        ...
        self.surface_to_embedding = nn.Linear(
            surface_size, embedding_dim, bias=False
        )  # surface_size → embedding_dim
        self.embedding_to_surface = nn.Linear(
            embedding_dim, surface_size, bias=False
        )  # embedding_dim → surface_size
        ...
        if config.adaptive_loss_weighting:
            ...
            self.register_buffer(
                ...
                torch.tensor(
                    [
                        self.weights["surface_reconstruction"],
    ...
    def forward(
        ...
        if "input_surface" in outputs and "output_surface" in outputs:
            ...
            if "target_surface" in targets:
                ...
                projected_input = self.embedding_to_surface(
                    input_surface
                )  # [batch, 225]
            ...
            surface_loss = torch.tensor(
                0.0, device=outputs["final_output"].device, requires_grad=True
            )
        ...
        if "final_output" in outputs and "target_embedding" in targets:
            ...
            if (
                target_embedding.shape[-1] == 4096
                and final_output.shape[-1] == expected_surface_size
            ):
                projected_target = self.embedding_to_surface(
                    target_embedding
                )  # [batch, surface_size]
                cos_sim = self.cosine_similarity(final_output, projected_target)
            elif (
                target_embedding.shape[-1] == expected_surface_size
                and final_output.shape[-1] == expected_surface_size
            ):
                ...
                projected_output = self.surface_to_embedding(
                    final_output
                )  # [batch, 4096]
        ...
        # Total weighted loss
        total_loss = (
            normalized_weights[0] * surface_loss
...
class EmergentSpatialPropagation(nn.Module):
    ...
    def __init__(
        ...
        self.propagation_gate = nn.Sequential(
            nn.Linear(cell_state_size * 2, cell_state_size), nn.Sigmoid()
        )
    ...
    def forward(self, cube_states: torch.Tensor) -> torch.Tensor:
        ...
        for layer_idx in range(depth - 1):
            current_layer = cube_states[
                :, layer_idx
            ]  # [batch, height, width, state_size]
            next_layer = cube_states[:, layer_idx + 1]

            # Flatten spatial dimensions for processing
            current_flat = current_layer.view(
                batch_size, -1, state_size
            )  # [batch, height*width, state_size]
            next_flat = next_layer.view(batch_size, -1, state_size)

            # Cross-layer influence
            connection_weights = self.layer_connections[
                layer_idx
            ]  # [state_size, state_size]
            influenced_next = torch.bmm(
                current_flat, connection_weights.unsqueeze(0).expand(batch_size, -1, -1)
            )

            # Gating mechanism
            combined = torch.cat(
                [next_flat, influenced_next], dim=-1
            )  # [batch, height*width, state_size*2]
            ...
            enhanced_states[:, layer_idx + 1] = enhanced_next.view(
                batch_size, height, width, state_size
            )
...
class EmergentGMLPCell(nn.Module):
    ...
    def forward(
        ...
        layer_context: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
        ...
        external_input_safe = (
            external_input.clone() if external_input is not None else None
        )

        base_output = self.base_gmlp(
            neighbor_states_safe, own_state_safe, external_input_safe
        )
        ...
        if neighbor_states_safe.numel() > 0:
            ...
            for i in range(neighbor_states_safe.shape[1]):  # For each neighbor
                ...
                combined = torch.cat(
                    [own_state_safe, neighbor_state], dim=-1
                )  # [batch, state_size*2]
                spatial_weights_input.append(combined)

            if spatial_weights_input:
                spatial_weights_input = torch.stack(
                    spatial_weights_input, dim=1
                )  # [batch, neighbor_count, state_size*2]

                # Generate adaptive weights для каждого neighbor
                spatial_weights = []
                for i in range(neighbor_states_safe.shape[1]):
                    weight = self.spatial_weight_generator(
                        spatial_weights_input[:, i]
                    )  # [batch, neighbor_count]
                    spatial_weights.append(
                        weight[:, i : i + 1]
                    )  # Take weight for this neighbor

                spatial_weights = torch.cat(
                    spatial_weights, dim=-1
                )  # [batch, neighbor_count]

                # Apply spatial weighting
                weighted_neighbors = neighbor_states_safe * spatial_weights.unsqueeze(
                    -1
                )  # [batch, neighbor_count, state_size]
                spatial_influence = torch.mean(
                    weighted_neighbors, dim=1
                )  # [batch, state_size]
        ...
        with torch.no_grad():
            ...
            current_activation = torch.mean(
                torch.abs(base_output.detach()), dim=0, keepdim=True
            )
...
class EmergentCubeTrainer(nn.Module):
    ...
    def __init__(
        self,
        config: Optional[EmergentTrainingConfig] = None,
        ...
        self.logger.info(
            f"   Spatial propagation depth: {self.config.spatial_propagation_depth}"
        )
    ...
    def _setup_enhanced_lattice(self):
        ...
        lattice_config = LatticeConfig(
            dimensions=self.config.cube_dimensions, boundary_conditions="walls"
        )
        ...
        self.cube_states_template = torch.zeros(
            1, depth, height, width, state_size, device=self._device
        )
        ...
        try:
            ...
            temp_permuted = self.cube_states_template.permute(
                0, 4, 1, 2, 3
            )  # [N, C, D, H, W]
            temp_channels_last = temp_permuted.contiguous(
                memory_format=torch.channels_last_3d
            )
            # Permute back to our format
            self.cube_states_template = temp_channels_last.permute(
                0, 2, 3, 4, 1
            )  # [N, D, H, W, C]
            self.logger.info(
    ...
    def _process_full_cube(self, cube_states: torch.Tensor) -> torch.Tensor:
        ...
        for cell_idx in range(total_cells):
            cell_state = flattened_states[
                :, cell_idx, :
            ].clone()  # [batch, state_size] - clone for safety

            # Get cell neighbors
            neighbors = self._get_cell_neighbors(
                cell_idx, flattened_states, batch_size, depth, height, width
            )

            # External input (zero for internal cells)
            external_input = torch.zeros(
                batch_size,
                self.config.gmlp_config["external_input_size"],
        ...
        processed_cube = processed_flattened.view(
                ...
                try:
                    ...
                    if nca_results is None:
                        ...
                        if hasattr(self, "_nca_metrics_cache"):
                            self._nca_metrics_cache.update(
                                nca_results.get("pattern_metrics", {})
                            )
                        else:
                            self._nca_metrics_cache = nca_results.get(
                                "pattern_metrics", {}
                            )
        ...
        for dd, dh, dw in directions:
            ...
            if 0 <= nd < depth and 0 <= nh < height and 0 <= nw < width:
                ...
                neighbor_state = torch.zeros(
                    batch_size,
                    self.config.gmlp_config["state_size"],
    ...
    def _inject_surface_to_cube(self, surface_embeddings: torch.Tensor) -> torch.Tensor:
        ...
        cube_states = torch.zeros(
            ...
            dtype=surface_embeddings.dtype,
        )
        ...
        if input_size == expected_surface_size:
            ...
            if (
                not hasattr(self, "surface_projection")
                or self.surface_projection.in_features != input_size
            ):
        ...
        for h in range(height):
            for w in range(width):
                ...
                cube_states[:, 0, h, w, 1:] = (
                    torch.randn(
                        batch_size, state_size - 1, device=surface_embeddings.device
                    )
                    * 0.01
                )
    ...
    def _extract_output_surface(self, cube_states: torch.Tensor) -> torch.Tensor:
        ...
        output_surface = surface_values.view(
            batch_size, expected_surface_size
        )  # [batch, height*width]
    ...
    def _analyze_internal_state(self, cube_states: torch.Tensor) -> torch.Tensor:
        ...
        if depth > 2:
            internal_layers = cube_states[
                :, 1:-1
            ]  # [batch, depth-2, height, width, state_size]

            # Average across internal layers
            internal_state = torch.mean(
                internal_layers, dim=1
            )  # [batch, height, width, state_size]

            # Flatten for loss computation
            internal_flattened = internal_state.view(
                batch_size, -1
            )  # [batch, height*width*state_size]
        else:
    ...
    def _detach_spatial_connections(self):
        ...
        for i, cell in enumerate(self.gmlp_cells):
            ...
            if (
                hasattr(cell.base_gmlp, "memory_state")
                and cell.base_gmlp.memory_state is not None
            ):
                # Создаем новое состояние вместо inplace операций
                cell.base_gmlp.memory_state = (
                    cell.base_gmlp.memory_state.detach().clone()
                )
    ...
    def _debug_tensor_versions(self, step_name: str):
        ...
        for i, cell in enumerate(self.gmlp_cells[:3]):  # Первые 3 для краткости
            if hasattr(cell, "specialization_tracker"):
                version = (
                    cell.specialization_tracker._version
                    if hasattr(cell.specialization_tracker, "_version")
                    else "N/A"
                )
                logger.debug(
        ...
        for i, cell in enumerate(self.gmlp_cells):
            ...
            if hasattr(cell, "specialization_tracker"):
                ...
                cell.register_buffer(
                    ...
                    torch.zeros(
                        1, state_size, device=cell.specialization_tracker.device
                    ),
                )
    ...
    def _lightweight_cleanup(self):
        ...
        if (
            hasattr(self.spatial_propagation, "last_layer_states")
            and len(self.spatial_propagation.last_layer_states) > 3
        ):
            # Очищаем только старые cached states (оставляем последние 3)
            keys_to_remove = list(self.spatial_propagation.last_layer_states.keys())[
                :-3
            ]
    ...
    def train_step_hierarchical(
        ...
        if (
            not hasattr(self.config, "gradient_accumulation_steps")
            or self.config.gradient_accumulation_steps <= 1
        ):
        ...
        for step in range(accumulation_steps):
            ...
            # Get mini-batch
            q_mini = (
                question_embeddings[start_idx:end_idx]
                .detach()
                .clone()
                .requires_grad_(True)
            )
            a_mini = (
                answer_embeddings[start_idx:end_idx]
                .detach()
                .clone()
                .requires_grad_(True)
            )
            ...
            for key, value in losses.items():
                ...
                if torch.is_tensor(value):
                    if value.numel() == 1:
                        ...
                        total_losses[key] += (
                            value.detach().mean().item() / accumulation_steps
                        )
                else:
        ...
        with torch.no_grad():
            ...
            try:
                if hasattr(self.loss_function, "embedding_to_surface"):
                    projected_answers = self.loss_function.embedding_to_surface(
                        answer_embeddings_sample
                    )
    ...
    def train_step(
        ...
        needs_full_reset = (
            self.training_step == 0  # Первый шаг всегда
            or self.training_step % 2 == 0  # ВРЕМЕННО: каждые 2 шага для диагностики
            or (
                hasattr(self, "_last_error_step")
                and self.training_step - self._last_error_step < 3
            )  # После ошибок
        )
        ...
        with torch.no_grad():
            question_embeddings = (
                question_embeddings.detach().clone().requires_grad_(True)
            )
        ...
        targets = {
            ...
            "target_surface": question_outputs["input_surface"]
            .detach()
            .clone(),  # Detach and clone reconstruction target
        }
    ...
    def _debug_computational_graph(
        self,
        outputs: Dict[str, torch.Tensor],
        ...
        for i, cell in enumerate(self.gmlp_cells[:3]):  # First 3 cells
            if (
                ...
                logger.error(
                    f"   Cell {i} memory_state: shape={mem_state.shape}, requires_grad={mem_state.requires_grad}"
                )
        ...
        if hasattr(self.spatial_propagation, "layer_connections"):
            ...
            logger.error(
                f"   Spatial connections: shape={conn.shape}, requires_grad={conn.requires_grad}"
            )
        ...
        if hasattr(self.loss_function, "surface_to_embedding"):
            ...
            for name, param in proj.named_parameters():
                logger.error(
                    f"   Loss projection {name}: requires_grad={param.requires_grad}, grad_fn={param.grad_fn}"
                )
        ...
        if hasattr(self, "surface_projection"):
            ...
            for name, param in self.surface_projection.named_parameters():
                logger.error(
                    f"   Surface proj {name}: requires_grad={param.requires_grad}"
                )
    ...
    def _ensure_device_consistency(self):
        ...
        if hasattr(self, "base_trainer"):
            ...
            if hasattr(self.base_trainer, "adapter") and hasattr(
                self.base_trainer.adapter, "to"
            ):
                self.base_trainer.adapter = self.base_trainer.adapter.to(self._device)
                components_moved.append("adapter")

            if hasattr(self.base_trainer, "cube_trainer") and hasattr(
                self.base_trainer.cube_trainer, "to"
            ):
                self.base_trainer.cube_trainer = self.base_trainer.cube_trainer.to(
                    self._device
                )
                components_moved.append("cube_trainer")

            if hasattr(self.base_trainer, "lattice") and hasattr(
                self.base_trainer.lattice, "to"
            ):
        ...
        if hasattr(self, "spatial_propagation") and hasattr(
            self.spatial_propagation, "to"
        ):
...
def create_emergent_trainer(
    ...
    logger.info(
        f"   Lattice: {info['total_cells']} cells × {info['avg_params_per_cell']:.0f} params"
    )
