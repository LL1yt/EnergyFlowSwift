...
file_handler = logging.FileHandler(
    "logs/real_training_production.log", encoding="utf-8"
)
...
# NEW: Import the refactored EmergentCubeTrainer
...
class ProductionTrainingManager:
    ...
    def __init__(
        self,
        config_path: str = "config/emergent_training_3_1_4_1.yaml",
        ...
        try:
            ...
            for stage in self.stages:
                ...
                if not stage_result["success"]:
                    logger.warning(
                        f"[WARNING]  Stage {stage.name} did not meet targets"
                    )
                    ...
                    if decision == "abort":
                        ...
                        logger.info(
                            f"[PLAY]  Continuing to next stage despite targets not met"
                        )
                    elif decision == "retry":
                        logger.info(
                            f"[REFRESH] Retrying stage {stage.name} with adjusted parameters"
                        )
    ...
    def _validate_system(self) -> bool:
        ...
        try:
            ...
            input_dim = test_embedding.shape[
                -1
            ]  # Get actual dimensions from test_embedding
            output_dim = 15 * 15  # Surface size for 15x15
            adapter = UniversalEmbeddingAdapter(
                input_dim=input_dim, output_dim=output_dim, strategy="hierarchical"
            )
            ...
            config.teacher_model = (
                self.model_name
            )  # Use selected model (DistilBERT or LLaMA)
            ...
            test_dialogue = [
                {
                    "question": "What is AI?",
                    "answer": "Artificial Intelligence is machine learning.",
                }
            ]
            ...
            if isinstance(sample, tuple):
                ...
                logger.info(
                    f"[OK] Training step successful: loss = {metrics.get('loss', 'N/A')}"
                )
    ...
    def _run_training_stage(self, stage: TrainingStage) -> Dict[str, Any]:
        ...
        for epoch in range(stage.epochs):
            ...
            for batch_idx in range(num_batches):
                try:
                    ...
                    batch_data = self._prepare_batch(
                        dataset, batch_idx, stage.batch_size
                    )
            ...
            epoch_similarity = (
                np.mean(epoch_similarities) if epoch_similarities else 0.0
            )
            ...
            if epoch_loss < stage_metrics["best_loss"]:
                ...
                if stage.save_checkpoints:
                    self._save_checkpoint(
                        trainer, stage, epoch, epoch_loss, epoch_similarity
                    )
            else:
                patience_counter += 1
            ...
            logger.info(
                f"    [DATA] Loss: {epoch_loss:.4f}, Similarity: {epoch_similarity:.4f}, Time: {epoch_time:.1f}s"
            )

            # Early stopping check
            if patience_counter >= stage.early_stopping_patience:
                logger.info(
                    f"    [STOP]  Early stopping triggered at epoch {epoch + 1}"
                )
            ...
            if (
                ...
                logger.info(
                    f"    [TARGET] Stage targets achieved at epoch {epoch + 1}!"
                )
        ...
        if (
            stage_metrics["best_similarity"]
            > self.training_history["best_metrics"]["similarity"]
        ):
            self.training_history["best_metrics"]["similarity"] = stage_metrics[
                "best_similarity"
            ]
        ...
        logger.info(
            f"    [TARGET] Best Loss: {stage_metrics['best_loss']:.4f} (target: {stage.target_loss})"
        )
        logger.info(
            f"    [TARGET] Best Similarity: {stage_metrics['best_similarity']:.4f} (target: {stage.target_similarity})"
        )
    ...
    def _get_medium_dialogue_dataset(self) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {
                "question": "Why use GPU training?",
                "answer": "GPUs have thousands of parallel cores optimized for matrix operations and simultaneous computation.",
            },
            {
                "question": "What is mixed precision?",
                "answer": "Training technique using both 16-bit and 32-bit floating point to reduce memory and accelerate training.",
            },
            {
                "question": "How does batch size affect training?",
                "answer": "Larger batches provide stable gradients and better GPU utilization but require more memory.",
            },
            {
                "question": "What is learning rate scheduling?",
                "answer": "Adjusting learning rate during training to improve convergence and avoid local minima.",
            },
            {
                "question": "What are optimization algorithms?",
                "answer": "Methods like Adam, SGD that update network parameters to minimize training loss.",
            },
        ]
    ...
    def _get_full_dialogue_dataset(self) -> List[Dict[str, str]]:
        ...
        advanced_pairs = [
            ...
            # Practical Applications
            {
                ...
                "answer": "Medical diagnosis, drug discovery, treatment planning, and personalized medicine applications.",
            },
            {
                "question": "What is computer vision?",
                "answer": "AI field enabling machines to interpret and understand visual information from images and videos.",
            },
            {
                "question": "How does natural language processing work?",
                "answer": "Computational techniques for analyzing, understanding, and generating human language text.",
            },
            {
                "question": "What are recommendation systems?",
                "answer": "AI systems that predict user preferences and suggest relevant content or products.",
            },
            {
                "question": "How is AI used in robotics?",
                "answer": "Enabling robots to perceive, plan, learn, and interact with physical environments.",
            },
        ]
    ...
    def _prepare_batch(
        self, dataset, batch_idx: int, batch_size: int
    ...
    def _save_checkpoint(
        self, trainer, stage: TrainingStage, epoch: int, loss: float, similarity: float
    ):
        """Сохранение checkpoint"""
        checkpoint_path = (
            self.checkpoint_dir / f"{stage.name}_epoch_{epoch}_loss_{loss:.4f}.pt"
        )
    ...
    def _analyze_stage_failure(
        self, stage: TrainingStage, result: Dict[str, Any]
    ) -> str:
        ...
        if stage.name == "validation":
            ...
            if best_loss > 2.0 or best_similarity < 0.05:
                logger.error(
                    "[ERROR] Critical validation failure - system not functional"
                )
                return "abort"
            else:
                logger.warning(
                    "[WARNING]  Validation targets not met but system functional"
                )
                return "continue"
            ...
            if len(metrics["losses"]) > 5:
                ...
                if improvement > 0.1:  # 10% improvement
                    logger.info(
                        f"[CHART] Learning detected ({improvement:.1%} loss improvement)"
                    )
                    return "continue"
                else:
    ...
    def _analyze_final_results(self) -> Dict[str, Any]:
        ...
        if performance_analysis["similarity_target_met"]:
            recommendations.append(
                "[SUCCESS] Similarity target achieved! System ready for production."
            )
        elif final_metrics["similarity"] >= 0.35:
            recommendations.append(
                "[CHART] Good progress made. Consider extended training or hyperparameter tuning."
            )
        else:
            recommendations.append(
                "[CONFIG] Limited progress. Review architecture, dataset quality, or training approach."
            )
        ...
        if performance_analysis["convergence_achieved"]:
            ...
            recommendations.append(
                "[WARNING]  Loss convergence incomplete. Consider longer training or learning rate adjustment."
            )
    ...
    def _create_training_visualizations(self):
        ...
        best_losses = [
            stage["metrics"]["best_loss"] for stage in self.training_history["stages"]
        ]
        best_similarities = [
            stage["metrics"]["best_similarity"]
            for stage in self.training_history["stages"]
        ]
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--llama":
            ...
            print(
                "Usage: python real_llama_training_production.py [--distilbert|--llama]"
            )
