...
class CellPrototype(nn.Module):
    ...
    def forward(self, 
        ...
        if neighbor_states.numel() > 0:  # Проверяем, что есть соседи
            ...
            neighbor_input = torch.zeros(batch_size, self.num_neighbors * self.state_size, 
                                       device=own_state.device, dtype=own_state.dtype)
        
        # Шаг 2: Подготавливаем внешний вход
        if external_input is None:
            external_input = torch.zeros(batch_size, self.input_size,
                                       device=own_state.device, dtype=own_state.dtype)
...
def create_cell_from_config(config: Dict[str, Any]) -> CellPrototype:
    """
    Создает экземпляр CellPrototype из конфигурации
    
    Параметры:
        config (Dict[str, Any]): Словарь с параметрами конфигурации
        
    Возвращает:
        CellPrototype: Настроенный экземпляр клетки
    """
    # Извлекаем параметры из конфигурации с значениями по умолчанию
    ...
    params = {
        ...
        'num_neighbors': 6,  # Для 3D решетки всегда 6 соседей
...
def test_cell_basic():
    ...
    neighbor_states = torch.randn(batch_size, num_neighbors, config['cell_prototype']['state_size'])
    ...
    print(f"    Диапазон значений: [{new_state.min().item():.3f}, {new_state.max().item():.3f}]")
    
    # Проверяем корректность
    assert new_state.shape == own_state.shape, f"Неверная форма выхода: {new_state.shape} vs {own_state.shape}"
    assert torch.all(torch.abs(new_state) <= 1.0), "Выходные значения должны быть в диапазоне [-1, 1]"
