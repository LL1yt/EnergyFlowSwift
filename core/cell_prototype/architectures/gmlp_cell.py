...
class GatedMLPCell(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_gating = SpatialGatingUnit(
            dim=hidden_dim,
            seq_len=neighbor_count + 1  # +1 для own state
        )
        ...
        self.input_residual = nn.Linear(total_input_size, state_size) if total_input_size != state_size else nn.Identity()
    ...
    def _log_parameter_count(self):
        ...
        logger.info(f"   Target: ~25,000 (current: {total_params:,})")
        
        if total_params > 30000:
            logger.warning(f"⚠️  Parameter count превышает target 25K: {total_params:,}")
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'target_parameters': 25000,
            'parameter_efficiency': total_params / 25000,
...
def test_gmlp_cell_basic() -> bool:
    ...
    try:
        ...
        # Проверки
        assert new_state.shape == (batch_size, 32), f"Wrong output shape: {new_state.shape}"
