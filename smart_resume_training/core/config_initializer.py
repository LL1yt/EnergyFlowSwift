...
class ConfigInitializer:
    ...
    def _initialize(self):
        ...
        if self.config_path:
            logger.info(f"[TOOL] PHASE 4: Using config_path: {self.config_path}")
            self._load_from_file()
        else:
            logger.info("[TOOL] PHASE 4: No config_path provided, generating dynamically")
    ...
    def _load_from_file(self):
        ...
        try:
            ...
            logger.info("[TOOL] PHASE 4: Successfully loaded config from file")
            ...
            if lattice_from_file:
                ...
                logger.info(f"[TOOL] PHASE 4: Lattice from file: {xs}×{ys}×{zs}")
    ...
    def _process_loaded_config(self, full_config: Dict):
        ...
        logger.info("[TOOL] PHASE 4: Processing loaded config...")
        ...
        if lattice_input:
            ...
            logger.info(f"[TOOL] PHASE 4: Input lattice: {xs_input}×{ys_input}×{zs_input}")
        ...
        if lattice_output:
            ...
            logger.info(
                f"[TOOL] PHASE 4: Output lattice: {xs_output}×{ys_output}×{zs_output}"
            )

        architecture_output = self.config.get("architecture", {})
        hybrid_mode_output = architecture_output.get("hybrid_mode", False)
        logger.info(f"[TOOL] PHASE 4: Output hybrid mode: {hybrid_mode_output}")
