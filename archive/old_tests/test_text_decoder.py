...
def test_embedding_cache():
    ...
    embeddings = [torch.randn(768), torch.randn(768), torch.randn(768)]

    texts = ["Hello world!", "This is a test sentence.", "Another example text."]
...
def test_joint_decoder():
    ...
    logger.info(
        f"  Logits shape: {results['logits'].shape if results['logits'] is not None else None}"
    )
...
def test_batch_processing():
    ...
    for batch_size in batch_sizes:
        ...
        assert (
            len(decoded_texts) == batch_size
        ), f"Batch size mismatch: {len(decoded_texts)} != {batch_size}"
...
def test_gpu_performance():
    ...
    for batch_size in large_batch_sizes:
        ...
        logger.info(
            f"  ✓ Decoded {batch_size} embeddings in {elapsed:.3f}s ({throughput:.1f} emb/s)"
        )
