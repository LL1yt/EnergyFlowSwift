...
def test_lattice_with_unified_optimizer():
    ...
    try:
        ...
        if optimizer_type != "UnifiedSpatialOptimizer":
            logger.error(
                f"❌ Ожидался UnifiedSpatialOptimizer, получен {optimizer_type}"
            )
            return False
        ...
        if hasattr(lattice.spatial_optimizer, "config"):
            ...
            logger.info(
                f"   🎯 Активный режим: {lattice.spatial_optimizer.active_mode.value}"
            )
        ...
        if "spatial_optimizer" in stats:
            ...
            logger.info(
                f"   📈 Статистика оптимизатора доступна: {len(spatial_stats)} компонентов"
            )

        # Получаем производительность
        if (
            hasattr(lattice, "perf_stats")
            and "spatial_optimization" in lattice.perf_stats
        ):
...
def test_performance_comparison():
    ...
    try:
        ...
        results["legacy"] = {"time_s": legacy_time, "type": "CPU Legacy"}
