...
def test_dialogue_dataset_basic():
    ...
    try:
        ...
        try:
            ...
            print(f"   [OK] Lattice size: {trainer.config.lattice_size}")
            ...
            print(f"   [OK] Forward pass test: {batch_input.shape} → {processed_embedding.shape}")
            ...
            print("   [TARGET] Teacher LLM архитектура полностью совместима с CubeTrainer!")
            
        except ImportError:
            print("   [WARNING]  CubeTrainer not available - skipping compatibility test")
        except Exception as e:
            print(f"   [WARNING]  CubeTrainer compatibility issue: {e}")
...
if __name__ == "__main__":
    ...
    if basic_success and advanced_success:
        ...
        print("[OK] Размеры куба [8,8,12] = 768D совместимы с DistilBERT")
        print("[OK] Batch обработка для CubeTrainer исправлена")
