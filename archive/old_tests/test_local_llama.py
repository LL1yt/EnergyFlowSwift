...
def test_llama_model():
    ...
    if os.path.exists(local_path):
        ...
        for key_file in key_files:
            if key_file in contents:
                ...
                print(f"[ERROR] Missing {key_file}")
    else:
        print("[ERROR] Local model directory does not exist")
        return False
    
    # 3. Пробуем создать LLM handler
    print(f"\n[BRAIN] Testing LLM Handler creation:")
    ...
    try:
        ...
        print(f"[OK] Test embedding generated: {test_embedding.shape}")
        print(f"   Dtype: {test_embedding.dtype}")
        
        return True
        
    except Exception as e:
        print(f"[ERROR] Error creating LLM handler: {e}")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n[SUCCESS] Local LLaMA-3-8B model is accessible!")
    else:
        print("\n[CONFIG] Local LLaMA-3-8B model needs configuration fixes") 