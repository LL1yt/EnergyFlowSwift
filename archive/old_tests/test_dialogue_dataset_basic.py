#!/usr/bin/env python3
"""
Базовый тест DialogueDataset - Stage 1.3
========================================
...
def test_dialogue_dataset_basic():
    ...
    try:
        ...
        try:
            ...
            # Создание тренера в dialogue режиме с правильными размерами куба
            training_config = TrainingConfig(
                        mode="dialogue",
                        device="cpu",
                        learning_rate=0.001,
                        embedding_dim=dataset.config.embedding_dim,
                        lattice_size=[8, 8, 12],  # 8*8*12 = 768 (совместимо с DistilBERT)
                        batch_size=2
                    )
