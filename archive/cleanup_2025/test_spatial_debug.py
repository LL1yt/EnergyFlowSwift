...
print(f"📊 Spatial optimizer: {type(optimizer)}")
print(f"📊 GPU processor: {type(optimizer.gpu_processor)}")

# Проверим adaptive_hash
adaptive_hash = optimizer.gpu_processor.adaptive_hash
print(f"📊 Adaptive hash: {type(adaptive_hash)}")
...
print(f"✅ Заполнили spatial hash для {total_cells} клеток")
    ...
    if (0 <= coords[0] < dims[0] and 
        ...
        for radius in [1.0, 1.5, 2.0, 3.0]:
            try:
                ...
                print(f"  📏 Радиус {radius}: найдено {len(neighbors)} соседей")
                if neighbors and len(neighbors) < 10:
                    ...
                    if center_idx in neighbors:
                        print(f"      ⚠️ Центральная точка {center_idx} все еще в списке!")
                    else:
                        print(f"      ✅ Центральная точка {center_idx} исключена")
            except Exception as e:
                print(f"  ❌ Ошибка при радиусе {radius}: {e}")
    else:
        print(f"⚠️ Координаты {coords} вне решетки {dims}")
