...
class EmbeddingProcessor(nn.Module):
        ...
        self.surface_modules = nn.ModuleDict(
            {
                ...
                "layer_transform": nn.Sequential(
                    nn.Linear(h * w, h * w), nn.Tanh(), nn.Linear(h * w, h * w)
                )
            }
        )
    ...
    def _process_through_lattice(self, matrix_3d: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            if self.config.processing_mode == ProcessingMode.AUTOENCODER:
                ...
                logger.warning(
                    "[WARNING]  _process_through_lattice вызван для SURFACE_ONLY режима. Используйте _surface_only_processing."
                )
    ...
    def _generator_processing(self, matrix_3d: torch.Tensor) -> torch.Tensor:
        ...
        transformed = matrix_3d * (
            1.0 + torch.randn_like(matrix_3d) * transformation_strength
        )
    ...
    def _update_metrics(
        self, input_batch: torch.Tensor, output_batch: torch.Tensor, start_time: float
    ):
        ...
        if self.config.verbose_logging:
            logger.info(
                f"[DATA] Cosine similarity: {avg_similarity:.3f} (цель: {self.config.target_similarity:.3f})"
            )
    ...
    def validate_quality(
        ...
        similarity = (
            torch.nn.functional.cosine_similarity(
                input_embedding,
                output_embedding,
                dim=0 if input_embedding.dim() == 1 else 1,
            )
            .mean()
            .item()
        )
    ...
    def _surface_only_processing(self, input_embedding: torch.Tensor) -> torch.Tensor:
        ...
        expected_surface_size = (
            self.config.surface_dimensions[0] * self.config.surface_dimensions[1]
        )
        if surface_size != expected_surface_size:
            logger.warning(
                f"Surface size mismatch: got {surface_size}, expected {expected_surface_size}"
            )
    ...
    def _standard_processing(self, input_embedding: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.debug_mode:
            logger.debug(
                f"[REFRESH] Этап 1: Преобразование {input_embedding.shape} → 3D"
            )
        ...
        for i in range(batch_size):
            ...
            matrix_3d = self.reshaper.vector_to_matrix(
                emb_1d
            )  # EmbeddingReshaper поддерживает torch
        ...
        if self.config.debug_mode:
            logger.debug(
                f"[REFRESH] Этап 3: Преобразование 3D → {self.config.output_dim}D"
            )
        ...
        for i in range(batch_size):
            ...
            emb_1d = self.reshaper.matrix_to_vector(
                matrix_3d
            )  # EmbeddingReshaper поддерживает torch
    ...
    def _expand_surface_to_volume(
        self, surface_2d: torch.Tensor, depth: int
    ) -> torch.Tensor:
        """
    ...
    def _spatial_diffusion(self, layer_2d: torch.Tensor) -> torch.Tensor:
        ...
        if h > 2 and w > 2:
            ...
            neighbors = (
                layer_2d[:-2, 1:-1]
                + layer_2d[2:, 1:-1]  # vertical neighbors
                + layer_2d[1:-1, :-2]
                + layer_2d[1:-1, 2:]  # horizontal neighbors
            ) / 4.0
    ...
    def _emergent_spatial_propagation(self, volume_3d: torch.Tensor) -> torch.Tensor:
        ...
        for step in range(self.config.propagation_steps):
            ...
            for layer in range(1, depth):
                ...
                new_result[layer] = beta * spatial_mixed + (1 - beta) * (
        ...
        if depth >= 3:
            ...
            for i in range(min(3, depth)):
                ...
                if i < extraction_weights.numel():
                    weighted_output += (
                        extraction_weights.flatten()[i] * volume_3d[layer_idx]
                    )
