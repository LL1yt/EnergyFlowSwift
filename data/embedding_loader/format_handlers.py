    ...
    def _load_text(self, path: str) -> np.ndarray:
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            if len(first_line.split()) == 2:
                ...
                logger.info(
                    f"Word2Vec info: vocab_size={vocab_size}, embed_dim={embed_dim}"
                )
            else:
    ...
    def _load_binary(self, path: str) -> np.ndarray:
        ...
        try:
            ...
            try:
                ...
                # Сохраняем словарь
                self._vocabulary = {
                    word: idx for idx, word in enumerate(model.index_to_key)
                }
                embeddings = model.vectors
                logger.info(
                    f"Loaded Word2Vec binary embeddings via gensim: {embeddings.shape}"
                )
                return embeddings

            except (ImportError, Exception) as e:
                logger.warning(
                    f"Gensim not available or failed ({e}), using alternative loader"
                )
        ...
        with open(path, "rb") as f:
            ...
            for i in range(vocab_size):
                ...
                if len(vector_bytes) < 4 * vector_dim:
                    logger.warning(
                        f"Incomplete vector for word '{word}', padding with zeros"
                    )
                    vector = np.zeros(vector_dim, dtype=np.float32)
                    available_floats = len(vector_bytes) // 4
                    if available_floats > 0:
                        vector[:available_floats] = struct.unpack(
                            f"{available_floats}f", vector_bytes[: available_floats * 4]
                        )
                else:
            ...
            logger.info(
                f"Successfully loaded Word2Vec binary (alternative): {embeddings.shape}"
            )
            return embeddings
...
class BertHandler(FormatHandler):
    ...
    def get_vocabulary(self, path: str) -> Dict[str, int]:
        ...
        """
        logger.warning(
            "BERT vocabulary not available from embedding file. "
...
class LLMHandler(FormatHandler):
        ...
        if self.model is None:
            try:
                ...
                self.tokenizer = AutoTokenizer.from_pretrained(
                    model_path,
                    cache_dir=(
                        LOCAL_MODELS_DIR if not os.path.exists(model_path) else None
                    ),
                )
                ...
                if model_path in [
                    SUPPORTED_LLM_MODELS[key] for key in LOCAL_MODEL_PATHS.keys()
                ]:
    ...
    def _get_model_path(self) -> str:
        ...
        if hasattr(self, "model_key") and self.model_key in LOCAL_MODEL_PATHS:
            ...
            if os.path.exists(local_path):
                ...
                logger.info(
                    f"Model {self.model_key} not cached yet, will download: {self.model_name}"
                )
                return self.model_name
        ...
        for model_key, local_path in LOCAL_MODEL_PATHS.items():
            if SUPPORTED_LLM_MODELS[model_key] == self.model_name:
                if os.path.exists(local_path):
                    ...
                    logger.info(
                        f"Model not cached yet, will download: {self.model_name}"
                    )
                    return self.model_name
        ...
        try:
            ...
            if hasattr(self, "model_key") and self.model_key in LOCAL_MODEL_PATHS:
                ...
                if not os.path.exists(local_path):
                    ...
                    logger.info(
                        f"Model {self.model_key} cached successfully: {local_path}"
                    )
                return
    ...
    def generate_embeddings(
        self, texts: List[str], pooling_strategy: str = "mean"
    ) -> torch.Tensor:
        """
    ...
    def batch_generate_embeddings(
        ...
        """
        # PHASE 4 FIX: Проверка на пустой список текстов
        if not texts:
            logger.error("batch_generate_embeddings: Empty text list provided")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # PHASE 4 FIX: Проверка каждого батча
            if not batch_texts:
        ...
        # PHASE 4 FIX: Проверка перед torch.cat
        if not all_embeddings:
            logger.error("No embeddings were generated - all batches were empty")
            raise RuntimeError(
                "torch.cat(): expected a non-empty list of Tensors - no valid embeddings generated"
            )

        logger.info(
            f"Successfully generated {len(all_embeddings)} batches of embeddings"
        )
...
LOCAL_MODELS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "models", "local_cache"
)
