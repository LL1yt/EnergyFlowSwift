...
try:
    ...
    print("💡 Убедитесь, что запускаете из корневой директории проекта")
    sys.exit(1)
...
class Simple2DLattice:
    ...
    def __init__(self, width=10, height=10, cell_config=None):
        ...
        print(f"✅ Создана 2D решетка {width}x{height} с общим прототипом клетки")
...
class Demo2DVisualizer:
    ...
    def create_animation(self, save_path=None, fps=10):
        ...
        if not self.lattice.history:
            print("❌ Нет истории для анимации. Запустите симуляцию сначала.")
            return
        ...
        if save_path:
            print(f"💾 Сохраняю анимацию в {save_path}...")
...
def main():
    ...
    """
    print("🎭 SIMPLE 2D DEMO - Клеточная Нейронная Сеть")
    print("=" * 60)
    print("""
    Эта демонстрация показывает базовые принципы работы нашей 3D системы
    на простом 2D примере. Каждая клетка в решетке:
    
    • 🧠 Получает сигналы от 4 соседей
    • ⚡ Обрабатывает их одинаковой нейросетью  
    • 📡 Передает результат дальше
    • 🔄 Все клетки обновляются синхронно
    ...
    print("=" * 30)
    print(f"✅ Проведено демонстраций: {len(demos)}")
    print("📁 Анимации сохранены в папке visualizations/")
    print("""
    💡 Что мы увидели:
    ...
    🚀 Следующий шаг: 3D версия с полным функционалом!
    """)
