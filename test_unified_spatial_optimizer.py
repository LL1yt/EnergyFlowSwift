...
def test_moe_integration():
    ...
    try:
        ...
        optimizer = create_unified_spatial_optimizer(
            dimensions, config, moe_processor
        )
    ...
    for dimensions in test_dimensions:
        try:
            ...
            for config_name, config in configs:
                ...
                logger.info(f"     💾 Общая память: {estimates['total_memory_gb']:.2f}GB")
                logger.info(f"     🎯 Рекомендуемая GPU: {estimates['recommended_gpu_memory_gb']:.2f}GB")
...
def test_performance_comparison():
    ...
    try:
        ...
        logger.info(f"   🚀 Unified: {unified_time:.3f}s avg, режим: {result.mode_used.value}")
