    ...
    def _create_hybrid_lattice(self):
        ...
        lattice_config = LatticeConfig(
            ...
            neighbor_strategy_config={
                "local_tier": {"ratio": 0.5, "radius": 3.0},    # NCA соседи
                "functional_tier": {"ratio": 0.3},              # gMLP соседи
                "long_range_tier": {"ratio": 0.2}               # gMLP дальние
            }
        )
...
integration_weights: [0.1, 0.9] # NCA:gMLP

cube_dimensions: [16, 16, 16] # next -> [666, 666, 333]
...
cell_config:
  ...
  neighbor_strategy_config:
    local_tier:
      ratio: 0.5 # 50% соседей для NCA
      radius: 3.0
    functional_tier:
      ratio: 0.3 # 30% соседей для gMLP
    long_range_tier:
      ratio: 0.2 # 20% дальних для gMLP
```
...
"""
Legacy compatibility wrapper для старого кода - нам не нужна никакая совместимость со старым кодом, так что можно смело удалять все лишнее. если есть сомнения, новая это часть, то лучше спросить
"""
...
def test_hybrid_nca_gmlp_cell():
    ...
    config = EmergentTrainingConfig.create_hybrid_config(
        nca_params={"state_size": 4, "target_params": 69},
        gmlp_params={"state_size": 8, "target_params": 23000},
...
_Создано на основе глубочайшего архитектурного анализа в COMPREHENSIVE_ARCHITECTURE_ANALYSIS_AND_IMPROVEMENT_PLAN.md_
