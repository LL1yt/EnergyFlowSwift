...
```python
{
    'energy_value': torch.Tensor,      # Текущая энергия/эмбеддинг; 768D эмбеддинг → маппер → каждый EnergyCarrier на входе получает долю от полного эмбеддинга
...
- ⚠️ Проблема: все потоки умирают от `energy_depleted` на первом шаге (требует оптимизации порога энергии)
