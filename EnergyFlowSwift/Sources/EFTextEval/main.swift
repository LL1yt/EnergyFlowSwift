...
func run() async throws {
    guard let args = parseArgs() else { usage(); return }
        ...
        func logProgress() {
            ...
            logger.info(String(format: "progress: seen=%d/ %d (%.1f%%) elapsed=%.2fs rate=%.1f samples/s",
                               seen,
                               expectedSamples,
                               expectedSamples > 0 ? 100.0 * Double(seen) / Double(expectedSamples) : 0,
                               dt,
                               rate),
                        category: Logger.Category.dataset)
        }
