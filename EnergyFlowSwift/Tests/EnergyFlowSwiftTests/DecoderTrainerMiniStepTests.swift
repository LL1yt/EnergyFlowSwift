...
final class DecoderTrainerMiniStepTests: XCTestCase {
    func test_projection_only_step_decreases_CE() async throws {
        ...
        let ce0 = try await trainer.step(ids: ids, zTeacher: zt, targets: tgts)
        let ce1 = try await trainer.step(ids: ids, zTeacher: zt, targets: tgts)
