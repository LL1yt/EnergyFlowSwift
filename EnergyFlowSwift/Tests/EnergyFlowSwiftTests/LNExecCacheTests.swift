...
final class LNExecCacheTests: XCTestCase {
    func testParitySmall() {
        ...
        let gpu = LNExecCache.shared.runForward(x: x, gamma: ln.gamma, beta: ln.beta, eps: ln.eps)
        XCTAssertEqual(cpu.shape, gpu.shape)
        for i in 0..<(N*D) {
            XCTAssertLessThan(abs(cpu.data[i] - gpu.data[i]), 1e-4)
        }
        // second call reuses executable
        _ = LNExecCache.shared.runForward(x: x, gamma: ln.gamma, beta: ln.beta, eps: ln.eps)
    }
}